...
import type { AddressInfo } from "@/types/trade";
...
onMounted(() => {
  store.trade.getAddressInfo();
  store.trade.getOrderInfo();
});
const { addressInfo, orderInfo } = storeToRefs(store.trade);
const changeDefault = (address: AddressInfo, addressInfo: AddressInfo[]) => {
  addressInfo.forEach((item) => (item.isDefault = "0"));
  address.isDefault = "1";
};
const userDefalutAddress = computed(() => {
  return addressInfo.value.find((item) => item.isDefault == "1");
});
const submitOrder = async () => {
  let submit = {
    consignee: userDefalutAddress.value?.consignee!,
    consigneeTel: userDefalutAddress.value?.consignee!,
    deliveryAddress: userDefalutAddress.value?.fullAddress!,
    paymentWay: "ONLINE",
    orderComment: msg.value,
    orderDetailList: orderInfo.value.detailArrayList,
  };
  let result: response = await reqSubmitOrder(orderInfo.value.tradeNo, submit);
  if (result.code == 200) {
