            ...
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd
                  ...
                  v-for="supSaleAttrValue in spuSaleAttr.spuSaleAttrValueList"
...
import type { SpuSaleAttrValueList } from "@/types/detail";
const route = useRoute();
const router = useRouter();
const store = useStore();
const skuNum = ref(1);
onMounted(() => {
  store.detail.getGoodsInfo(route.params.skuId as string);
});
const { categoryView, skuInfo, spuSaleAttrList } = storeToRefs(store.detail);
const changeActivity = (
  saleAttrValue: SpuSaleAttrValueList,
  arr: SpuSaleAttrValueList[]
) => {
  arr.forEach((item: SpuSaleAttrValueList) => {
    item.isChecked = "0";
  });
...
const addShopCar = async () => {
  try {
    await store.detail.addOrUpdateShopCart(
      route.params.skuId as unknown as number,
      skuNum.value
    );
    sessionStorage.setItem("SKUINFO", JSON.stringify(skuInfo.value));
    router.push({
      name: "addcartsuccess",
      params: { skuNum: skuNum.value },
    });
  } catch (error: any) {
    alert(error.message);
  }
